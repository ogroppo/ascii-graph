...
const {FIRST_CHILD, LAST_CHILD, FIRST_PARENT, LAST_PARENT, HALF_RIGHT, HALF_LEFT, DASH, CHEVRON, VERTICAL} = require('./symbols')
...
module.exports = class AsciiTree{
  ...
  mapParents(parents, originRow = 0, parentDepth = -1, currentLeft = 0){
    ...
    //make space
    if(parents.length > 1){
      let halfOverflow = Math.floor(halfParentsLength)
      if(originRow < 0){
        this.moveTopParents(originRow, halfOverflow)
        originRow -= halfOverflow
        this.moveTopParents(originRow - halfOverflow, halfOverflow)
      }
      if(originRow > 0){
        this.moveBottomParents(originRow, halfOverflow)
        originRow += halfOverflow
        this.moveBottomParents(originRow + halfOverflow, halfOverflow)
      }
      if(originRow === 0){
        this.moveTopParents(-1, halfOverflow)
        this.moveBottomParents(1, halfOverflow)
      }
    }
  ...
  moveTopChildren(fromRow, moveAmount){
    this.elements.forEach((connector, connectorIndex) => {
      ...
      if(connector.depth > 0 && connector.row <= fromRow){
        if(connector.row === fromRow &&
           !connector.display.startsWith(LAST_CHILD) &&
           !connector.display.startsWith(DASH)
         ){
          //add vertical pipes
  ...
  moveTopParents(fromRow, moveAmount){
    this.elements.forEach((element, elementIndex) => {
      ...
      if(element.depth < 0 && element.row <= fromRow){
        if(element.row === fromRow && //only once
          !element.display.endsWith(LAST_PARENT + ' ') && //not for last parent
          !element.display.endsWith(DASH + CHEVRON) //nor single parent
        ){
    ...
    this.elements.forEach((element, elementIndex) => {
      if(element.depth > 0 && element.row >= fromRow){
        if(element.row === fromRow && //add vertical pipes, only once
          ...
          !element.display.startsWith(DASH) //nor single child
        ){
          for (var i = 0; i < moveAmount; i++) {
            this.elements.push({
              display: VERTICAL,
    ...
    this.elements.forEach((element, elementIndex) => {
      ...
      if(element.depth < 0 && element.row >= fromRow){
        ...
        if(element.row === fromRow && //only for one element (once)
          ...
          !element.display.endsWith(DASH + CHEVRON)
        ){
          for (var i = 0; i < moveAmount; i++) {
            this.elements.push({
              display: VERTICAL + ' ',
